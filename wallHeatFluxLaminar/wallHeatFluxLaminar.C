...
#       include "createFields.H"
        ...
        surfaceScalarField heatFlux = tc*fvc::snGrad(T);
        
//        const surfaceScalarField::GeometricBoundaryField& patchHeatFlux = heatFlux.boundaryField();
// use boundaryFieldRef() here instead of boundaryField

        surfaceScalarField::Boundary& patchHeatFlux = heatFlux.boundaryFieldRef();
...
*/
        ...
        (
         ...
         Info<<"heatFlux.dimensions()" << heatFlux.dimensions() << endl;

        forAll(wallHeatFlux.boundaryFieldRef(), patchi)
        {
        Info<< "\nThis is boundaryFieldRef" << "\n" << wallHeatFlux.boundaryFieldRef() << endl;
        Info<< "\nThis is Patchi" << "\n" << patchi << endl;

      ...
      {            
          if ( float(Cpatches[cellj].x()) == float(mesh.C()[celli].x()) )  
          {
       	...
       	forAll(faces, facei)
       	{
       		...
       		{
       		T_flux_list_for_this_section.append(phi[faces[facei]]);
...
//    Info<<"T_flux_listlist" << T_flux_listlist<< endl; 
    ...
    List<float> SUM_T_flux_list;
    forAll(T_flux_listlist, i)

    {
    SUM_T_flux_list.append(std::accumulate(T_flux_listlist[i].begin(), T_flux_listlist[i].end(), 0.0f));
    }
//    Info<<"SUM_T_flux_list" << SUM_T_flux_list<< endl;   

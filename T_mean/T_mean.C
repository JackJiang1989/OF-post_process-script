...
//        test1.append([1.1]);
	...
	List<List<float>> T_flux_listlist;  
...
//        label nSelectedCell = 1; //if this SelectedCell really necessary? can we just use cellj instead???
	...
	List<float> T_flux_list_for_this_section;         
...
//    Info<<"flux_listlist" << T_flux_listlist<< endl;   
    
//    Info<<"T_flux_listlist" << T_flux_listlist<< endl; 

    List<float> avg_T_list;
    List<float> total_flux_list;    
    std::ofstream output_file; 
    
    
    forAll(T_flux_listlist, i)
    {
    double sum_T_flux= double(std::accumulate(T_flux_listlist[i].begin(), T_flux_listlist[i].end(), 0.0f));
    double sum_flux= double(std::accumulate(flux_listlist[i].begin(), flux_listlist[i].end(), 0.0f));
    if (sum_flux != 0)
    {
	    avg_T_list.append(sum_T_flux/sum_flux);
	    total_flux_list.append(sum_flux);
	    
    }
//    Info<<"sum_T_flux" << sum_T_flux<< endl;  
//    Info<<"sum_flux" << sum_flux<< endl;  
//    Info<<"DIV" << DIV << endl;
//    avg_T_list.append(sum_T_flux/sum_flux);
    }
    Info<<"avg_T_list" << avg_T_list<< endl;
    Info<<"total_flux_list" << total_flux_list<< endl;     
    Info<<"T_wall" << T.boundaryFieldRef()<< endl;      
//    output_file.open("./test.txt");    
//    output_file << avg_T_list;    
//    output_file.close();    
//    Info<<"avg_T_list" << avg_T_list<< endl;   
